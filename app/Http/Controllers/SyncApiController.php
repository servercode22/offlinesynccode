...
class SyncApiController extends Controller
{
    ...
    public function businessRegValidate(request $request)
    {
        ...
        $live_server_path = "http://possyns.crossdevlogix.com/connector/api/";
    ...
    public function syncSale()
    {
        ...
        try {
            ...
            $url = "http://possyns.crossdevlogix.com/api/";
            ...
            $request = $http->post('sync-start-time', [
                'query' => [
                    'business_id' => $business_id,
            ...
            if ($request->getStatusCode() == 200) {
                $row_data = (array) json_decode($request->getBody());
                // return $row_data;
            ...
            $request = $http->post('sync-end-time', [
                'query' => [
                    'business_id' => $business_id,
            ...
            if ($request->getStatusCode() == 200) {
                $bodyData = (array) json_decode($request->getBody());
                if (isset($bodyData['success'])) {
                ...
                throw new Exception("Sync Failed Status not 200");
            }
        } catch (Exception $e) {
            DB::rollBack();
            dd($e);
    ...
    function test()
    {
        ...
        $url = "http://possyns.crossdevlogix.com/connector/api/";
        $http = new Client([
            'base_uri' => $url,
            'headers' => [
                'Content-Type' => 'application/json', 'Accept' => 'application/json',
